...
FR1.3: Authentication will have a flow on chatbot and involve microsoft Authenticator, this can be linked with a flow on the Frontend. 
### 2. Objectives & Points
FR2.1: Users are assigned sales or generic objectives from Data Lakehouse, this need to be aggregated and assigned as progress or create new objectives if not present
...
FR2.4: Objectives may be updated or marked complete via data aggregated or internal events (e.g., video watched, event completed). 
...
### 6. Flash Events / Campaigns
FR6.1: (On Hold) Admins can create temporary events (e.g., send a photo). 
FR6.2: Events/Campaigns can be configured with point rewards upon successful user completion. 
...
FRW1.4: (On Hold) Users can submit flash event participation (e.g., send photo, text response). 
